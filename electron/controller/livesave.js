...
class LiveMonitorController {
     ...
     * 获取录制设置
     */
    async getRecordSettings(args, event) {
        try {
            logger.info('获取录制设置');
            const result = await liveMonitorService.readGeneralConfig();
            ...
            if (result.status === 'success') {
                return {
                    ...
                    status: 'success',
                    message: '获取录制设置成功',
                    data: result.data.recordSettings
                };
            } else {
                return {
                    success: false,
     ...
     * 更新录制设置
     */
    async updateRecordSettings(args, event) {
        try {
            logger.info(`更新录制设置: ${JSON.stringify(args)}`);
            ...
            const settingsToUpdate = {
                ...
                segmentEnabled: args.segmentEnabled === true,
                segmentTime: parseInt(args.segmentTime || 1800),
                convertToMp4: args.convertToMp4 === true
            };

            logger.info(`处理后的设置参数: ${JSON.stringify(settingsToUpdate)}`);
            const result = await liveMonitorService.updateGeneralConfig(settingsToUpdate);
